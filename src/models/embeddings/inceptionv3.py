...
class BasicConv2d(nn.Module):
    ...
    def __init__(self, input_channels, output_channels, **kwargs):
        ...
        self.bn = nn.BatchNorm2d(output_channels, track_running_stats=False)
...
class InceptionV3_224(nn.Module):
    ...
    def __init__(self, latent_size=512):#num_classes=100):
        super().__init__()
        ...
        self.linear = nn.Linear(2048, self.latent_size)#num_classes)

        self.batch_norm_1 = nn.BatchNorm1d(self.latent_size)
        
        #self.fc1 = nn.Linear(1024, 512)
        #self.batch_norm_2 = nn.BatchNorm1d(512)
        #self.fc2 = nn.Linear(512, self.latent_size)
        #self.act = nn.PReLU()
        #self.fc3 = nn.Linear(self.latent_size, 5)
    ...
    def forward(self, x):
        ...
        x = self.dropout(x)
        x = x.view(x.size(0), -1)
        #x = self.batch_norm_1(self.linear(x))
        x = self.linear(x)
        x = self.batch_norm_1(x)

        #x = self.fc1(self.act(x))
        #x = self.fc1(self.act(x))
        #out = self.fc2(self.act(x))
        #out2 = self.fc3(out)
        return x
...
class InceptionV3_299(nn.Module):
    def __init__(self, latent_size=128):
        super(InceptionV3_299, self).__init__()
        self.name = 'InceptionV3'
        self.latent_size = latent_size
        self.convnet = models.inception_v3(pretrained=True)
        self.convnet.aux_logits = False
        self.convnet.fc = nn.Linear(2048, 1024)
        self.fc1 = nn.Linear(1024, 512)
        self.fc2 = nn.Linear(512, self.latent_size)
        self.act = nn.PReLU()
